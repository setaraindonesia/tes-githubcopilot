...
export class EmailService {
  ...
  public async sendPasswordResetEmail(recipientEmail: string, token: string): Promise<void> {
    if (!this.resend) {
      this.logger.warn('RESEND_API_KEY is not set; skipping sending password reset email.');
      return;
    }

    const resetUrl = `${this.appWebUrl}/reset-password?token=${encodeURIComponent(token)}&email=${encodeURIComponent(recipientEmail)}`;

    const subject = 'Reset Password Akun Setaradapps';
    const html = `
      <div style="font-family: Arial, Helvetica, sans-serif; line-height: 1.6; color: #0f172a;">
        <h2 style="margin: 0 0 12px; color: #111827;">Reset Password</h2>
        <p style="margin: 0 0 16px;">Gunakan <strong>kode 6 digit</strong> berikut untuk reset password Anda.</p>
        <div style="font-size:28px;letter-spacing:8px;font-weight:700;background:#f3f4f6;padding:12px;border-radius:8px;text-align:center;color:#111827;">${token}</div>
        <p style="margin: 16px 0 8px;">Atau klik tombol di bawah ini:</p>
        <p style="margin: 0 0 16px;">
          <a href="${resetUrl}" style="display:inline-block;padding:10px 16px;background:#0066FF;color:#ffffff;text-decoration:none;border-radius:8px;">Reset via Tautan</a>
        </p>
        <p style="font-size:12px;color:#6b7280;margin-top:16px;">Jika Anda tidak meminta reset, abaikan email ini.</p>
      </div>
    `;
    ...
    try {
      ...
      this.logger.log(`Password reset email enqueued for ${recipientEmail} (id: ${result?.data?.id ?? 'n/a'})`);
    } catch (error) {
      this.logger.error(`Failed sending password reset email to ${recipientEmail}`, error as Error);
    }
  }
}
